...
bool MatrixBase<Derived>::isUpperTriangular( RealScalar prec ) const
{
    ...
    for ( int j = 0; j < cols(); ++j )
        for ( int i = 0; i <= j; ++i )
        {
            ...
            if ( absValue > maxAbsOnUpperTriangularPart )
                maxAbsOnUpperTriangularPart = absValue;
        }
    for ( int j = 0; j < cols() - 1; ++j )
        for ( int i = j + 1; i < rows(); ++i )
            if ( !ei_isMuchSmallerThan( coeff( i, j ), maxAbsOnUpperTriangularPart, prec ) )
                return false;
    return true;
}
...
bool MatrixBase<Derived>::isLowerTriangular( RealScalar prec ) const
{
    ...
    for ( int j = 0; j < cols(); ++j )
        ...
        {
            RealScalar absValue = ei_abs( coeff( i, j ) );
            if ( absValue > maxAbsOnLowerTriangularPart )
                maxAbsOnLowerTriangularPart = absValue;
        }
    for ( int j = 1; j < cols(); ++j )
        for ( int i = 0; i < j; ++i )
            if ( !ei_isMuchSmallerThan( coeff( i, j ), maxAbsOnLowerTriangularPart, prec ) )
                return false;
    return true;
}
