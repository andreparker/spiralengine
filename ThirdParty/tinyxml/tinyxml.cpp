...
void TiXmlElement::Print(FILE_INTERFACE	*cfile,	int	depth)const
{
	int	i;
	for	(i = 0;	i	<	depth; i++)
	{
	...
	fi_fprintf(cfile, "<%s",	value.c_str());
...
bool TiXmlDocument::LoadFile(const char	*filename,void *mem,int len,TiXmlEncoding encoding)
{
	...
	FILE_INTERFACE *file = fi_fopen(value.c_str(),	"rb", mem, len);
...
bool TiXmlDocument::LoadFile(FILE_INTERFACE	*file, TiXmlEncoding encoding)
{
	if (!file)
	{
		SetError(TIXML_ERROR_OPENING_FILE, 0,	0, TIXML_ENCODING_UNKNOWN);
	...
	long length	=	0;
	fi_fseek(file,	0, SEEK_END);
	length = fi_ftell(file);
	fi_fseek(file,	0, SEEK_SET);
	...
	if (fi_fread(buf, length, 1,	file)	!= 1)
	{
		SetError(TIXML_ERROR_OPENING_FILE, 0,	0, TIXML_ENCODING_UNKNOWN);
		return false;
	}
...
bool TiXmlDocument::SaveFile(const char	*filename)const
{
	...
	FILE_INTERFACE *fp = fi_fopen(filename, "w");
	if (fp)
	{
...
bool TiXmlDocument::SaveFile(FILE_INTERFACE	*fp)const
{
	if (useMicrosoftBOM)
	{
		...
		fi_fputc(TIXML_UTF_LEAD_0,	fp);
		fi_fputc(TIXML_UTF_LEAD_1,	fp);
		fi_fputc(TIXML_UTF_LEAD_2,	fp);
	}
	Print(fp,	0);
	return true;
}
...
void TiXmlDocument::Print(FILE_INTERFACE *cfile, int depth)const
{
	const	TiXmlNode	*node;
	for	(node	=	FirstChild();	node;	node = node->NextSibling())
	{
...
void TiXmlAttribute::Print(FILE_INTERFACE	*cfile,	int	/*depth*/)const
{
	TIXML_STRING n,	v;
	...
	{
		fi_fprintf(cfile, "%s=\"%s\"",	n.c_str(), v.c_str());
	}
	else
	{
		fi_fprintf(cfile, "%s='%s'",	n.c_str(), v.c_str());
...
void TiXmlComment::Print(FILE_INTERFACE	*cfile,	int	depth)const
{
...
void TiXmlText::Print(FILE_INTERFACE *cfile, int depth)const
{
	if (cdata)
	{
		int	i;
		...
		fi_fprintf(cfile, "<![CDATA[");
		fi_fprintf(cfile, "%s", value.c_str()); //	unformatted	output
		fi_fprintf(cfile, "]]>\n");
	}
	else
	{
		...
		fi_fprintf(cfile, "%s", buffer.c_str());
	}
}
...
void TiXmlDeclaration::Print(FILE_INTERFACE	*cfile,	int	/*depth*/)const
{
	fi_fprintf(cfile, "<?xml	");

	if (!version.empty())
	{
		fi_fprintf(cfile, "version=\"%s\" ",	version.c_str());
	}
	if (!encoding.empty())
	{
		fi_fprintf(cfile, "encoding=\"%s\"	", encoding.c_str());
	}
	if (!standalone.empty())
	{
		fi_fprintf(cfile, "standalone=\"%s\"	", standalone.c_str());
	}
	fi_fprintf(cfile, "?>");
}
...
void TiXmlUnknown::Print(FILE_INTERFACE	*cfile,	int	depth)const
{
	for	(int i = 0;	i	<	depth; i++)
	{
		fi_fprintf(cfile, "		");
