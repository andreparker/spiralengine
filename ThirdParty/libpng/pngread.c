                                ...
                                if ( !png_memcmp( chunk_name, png_cHRM, 4 ) )
                                    ...
                                    if ( !png_memcmp( chunk_name, png_gAMA, 4 ) )
                                        ...
                                        if ( !png_memcmp( chunk_name, png_hIST, 4 ) )
                                            ...
                                            if ( !png_memcmp( chunk_name, png_oFFs, 4 ) )
                                                png_handle_oFFs( png_ptr, info_ptr, length );
#endif
#if defined(PNG_READ_pCAL_SUPPORTED)
                                            else
                                                if ( !png_memcmp( chunk_name, png_pCAL, 4 ) )
                                                    png_handle_pCAL( png_ptr, info_ptr, length );
#endif
#if defined(PNG_READ_sCAL_SUPPORTED)
                                                else
                                                    if ( !png_memcmp( chunk_name, png_sCAL, 4 ) )
                                                        png_handle_sCAL( png_ptr, info_ptr, length );
#endif
#if defined(PNG_READ_pHYs_SUPPORTED)
                                                    else
                                                        if ( !png_memcmp( chunk_name, png_pHYs, 4 ) )
                                                            ...
                                                            if ( !png_memcmp( chunk_name, png_sBIT, 4 ) )
                                                                ...
                                                                if ( !png_memcmp( chunk_name, png_sRGB, 4 ) )
                                                                    ...
                                                                    if ( !png_memcmp( chunk_name, png_iCCP, 4 ) )
                                                                        ...
                                                                        if ( !png_memcmp( chunk_name, png_sPLT, 4 ) )
                                                                            png_handle_sPLT( png_ptr, info_ptr, length );
#endif
#if defined(PNG_READ_tEXt_SUPPORTED)
                                                                        else
                                                                            if ( !png_memcmp( chunk_name, png_tEXt, 4 ) )
                                                                                png_handle_tEXt( png_ptr, info_ptr, length );
#endif
#if defined(PNG_READ_tIME_SUPPORTED)
                                                                            else
                                                                                if ( !png_memcmp( chunk_name, png_tIME, 4 ) )
                                                                                    ...
                                                                                    if ( !png_memcmp( chunk_name, png_tRNS, 4 ) )
                                                                                        ...
                                                                                        if ( !png_memcmp( chunk_name, png_zTXt, 4 ) )
                                                                                            png_handle_zTXt( png_ptr, info_ptr, length );
#endif
#if defined(PNG_READ_iTXt_SUPPORTED)
                                                                                        else
                                                                                            if ( !png_memcmp( chunk_name, png_iTXt, 4 ) )
