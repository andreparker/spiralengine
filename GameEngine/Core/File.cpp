...
wString ConvertByteToWide( const char* str )
{
	wString converted;
	while( *str != NULL )
	{
...
boost::shared_ptr< std::wistream > Spiral::IFile::GetStreamW()
{
	shared_ptr< std::wistream > stream;
	if( !DoEof() )
	{
		uint32_t size = DoSize();
		scoped_array< char > array( new char[ size + 1 ] );
		...
		stream.reset( new std::wistringstream( ConvertByteToWide( array.get() ) ) );
	}
