...
private:
    ...
    template <class ForwardEvent>
    typename boost::enable_if<typename boost::is_same<ForwardEvent,Event>::type,void >::type
        helper(ForwardEvent const& incomingEvent)
    {
    ...
    template <class ForwardEvent>
    typename boost::disable_if<typename boost::is_same<ForwardEvent,Event>::type,void >::type
        helper(ForwardEvent const& incomingEvent)
    {
 ...
 protected:    // interface for the derived class
    ...
    struct row
    {
        ...
        static std::pair<int,HandledEnum> execute(Derived& fsm, int state, pstate_base* all_states, Event const& evt)
        {
            ...
            execute_entry<next_state_type>(all_states[next_state].get(),evt);
...
private:
    ...
    struct add_state
    {
        ...
        typename boost::enable_if<
            typename mpl::and_<typename is_composite_state<StateType>::type,
                               typename has_exit_pseudo_states<StateType>::type >::type,state_base* >::type
        new_state_helper() const
        {
        ...
        typename boost::enable_if<
            	typename mpl::and_<typename is_composite_state<StateType>::type,
                               	   typename mpl::not_<typename has_exit_pseudo_states<StateType>::type>::type
        ...
        // state is a normal state and needs nothing except creation
        template <class StateType>
        typename boost::disable_if<
            typename mpl::or_<typename is_composite_state<StateType>::type,
                              typename is_pseudo_exit<StateType>::type >::type,state_base* >::type
        new_state_helper() const
        {
            // just create without arguments
            return new StateType;
        }
        ...
        void operator()(boost::msm::wrap<State> const&) const
        {
            ...
            pstate_base new_state (new_state_helper<State>());
