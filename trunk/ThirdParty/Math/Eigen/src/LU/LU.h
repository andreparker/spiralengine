...
void LU<MatrixType>::computeKernel( KernelMatrixType *result ) const
{
    ...
    for ( int i = m_rank; i < cols; ++i )
        result->row( m_q.coeff( i ) ).setZero();
    for ( int k = 0; k < dimker; ++k )
        result->coeffRef( m_q.coeff( m_rank + k ), k ) = Scalar( 1 );
}
...
) const
{
    ...
    for ( int i = 0; i < m_rank; ++i )
        result->row( m_q.coeff( i ) ) = c.row( i );
    for ( int i = m_rank; i < m_lu.cols(); ++i )
        result->row( m_q.coeff( i ) ).setZero();
    return true;
}
