...
bool App::DoInit( boost::int32_t /*argc*/, std::list< boost::shared_array< char > >& /*argList*/, boost::shared_ptr< Engine >& engine )
{
    ...
    shared_ptr< IFile > testlayout;

    if ( FileManager::instance().getFile( "Data/GUI/layouts/window.layout", testlayout ) )
    {
        engine->GetGuiManager()->LoadLayout( testlayout );
        testlayout->Close();
    }
    ...
    m_camera = new Camera( Math::Vector3f( 0.0f, 0.0f, 0.0f ) );
